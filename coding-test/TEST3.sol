...
contract TEST3 {
    ...
    */
    ...
    modifier loginAttempts(string memory _id) {
        _;
        require(users[_id].attempt < 5, "Please reenter your password.");
    }

    function signIn(string memory _id, string memory _pw) public loginAttempts(_id) returns(bool) {
        require(!checkIdPw(_id, _pw), "Invalid user");
        bytes32 userHash = keccak256(abi.encodePacked(_id, _pw));

        if (users[_id].userHash != userHash) {
            users[_id].attempt++;
            return false;
        } 
    ...
    function signUp(string memory _id, string memory _pw) public {
        require(checkIdPw(_id, _pw), "ID already exists");
        users[_id] = User(keccak256(abi.encodePacked(_id, _pw)), 0);
    }

    function deleteUser(string memory _id, string memory _pw) public {
        checkIdPw(_id, _pw);
        bytes32 userHash = keccak256(abi.encodePacked(_id, _pw));
        require(users[_id].userHash == userHash, "User ID or password is incorrect.");
        delete users[_id];
    }

    function checkIdPw(string memory _id, string memory _pw) public view returns(bool) {
        require(bytes(_id).length > 0, "Invalid Id");
        require(bytes(_pw).length > 0, "Invalid Pw");
        return users[_id].userHash == 0;
    }
}