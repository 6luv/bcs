...
contract Q52 {
    ...
    */
    ...
    function getId(string memory _name, uint _birth) public view returns(bytes10) {
...
contract Q53 {
    ...
    */
    ...
    function deposit(string memory _bank) public payable {
        require(bytes(_bank).length == 1 && bytes1("A") <= bytes(_bank)[0] && bytes(_bank)[0] <= bytes1("E"));
    ...
    function withdraw(string memory _bank, uint _balance) public {
        require(balances[_bank][msg.sender] >= _balance);
...
contract Q56 {
    ...
    */
    ...
    function changeOwner(address _addr) public {
        require(agreeOwner && agreeSubOwner);
        owner = _addr;
        agreeOwner = false;
        agreeSubOwner = false;
    }
    ...
    function approveSubOwner() public {
        require(subOwner == msg.sender);
        agreeSubOwner = true;
    }
}
...
contract Q57 {
    ...
    */
    ...
    function changeOwner(address _addr) public {
        require(owner == msg.sender || msg.sender == subOwner && agreeOwner);
        owner = _addr;
    }
...
contract Q60 {
    ...
    */
    ...
    mapping(uint => mapping(uint => address[3])) reservations;

    function setReservation(uint _date, uint _room, address[] memory _guests) public {
        require(_room == 1 || _room == 2);
        (bool isFull, uint index) = checkRoom(_date, _room);
        require(index + _guests.length <= 3 && !isFull);

        for (uint i = 0; i < _guests.length; i ++) {
            reservations[_date][_room][index++] = _guests[i];
        }
    }

    function checkRoom(uint _date, uint _room) internal view returns(bool, uint) {
        for (uint i = 0; i < 3; i ++) {
            if (reservations[_date][_room][i] == address(0)) {
                return (false, i);
            }
        }
        return (true, 0);
    ...
    function getReservation(uint _date, uint _room) public view returns(address[3] memory) {
